...
pROC_train <- roc(qualityTrain$PoorCare, predictTrain)
...
```{r}
accuracy_perf <- performance(ROCRpred, measure = "acc")
plot(accuracy_perf)
```
...
AUC_test <- performance(ROCRpredTest, measure = "auc")
AUC_test@y.values
```
...
```{r}
auc(qualityTest$PoorCare, predictTest)
```
